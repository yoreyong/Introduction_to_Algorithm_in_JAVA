...
public class BFS extends Graph.GraphAlgorithm<BFS.BFSVertex> {
    ...
    public void bfs(Vertex src) {
        this.src = src;
        // code for BFS
        // initialize vertex properties of all vertices
        for(Vertex u : g) {
            get(u).seen = false;
            get(u).parent = null;
        ...
        while(!q.isEmpty()) {
            ...
            for(Edge e : g.incident(u)) {
                Vertex v = e.otherEnd(u);   // find the other vertex connected to u
                if(!get(v).seen) {
                    get(v).seen = true;
                    get(v).parent = u;
                    get(v).distance = get(u).distance + 1; // store the distance
                    q.add(v);
                }
            }
        }
